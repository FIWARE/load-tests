...
class EntityUpdateEverythingSubscriptionSimulation extends EntityUpdateSimulation {

  override def beforeScenario(): Unit = {
    val subscription = getEverythingSubscriptionAction(testConfig.notificationServerUrl)
  ...
  override def afterScenario(): Unit = {
    Http(baseUrl + "subscriptions/urn:ngsi-ld:Subscription:everything").header("Content-Type", "application/ld+json").method("DELETE").timeout(1000, 6000).asString
  }
